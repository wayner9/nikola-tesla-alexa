...
if len(sys.argv) <3:
    sys.exit ("You must pass an email and password as arguments when starting this script")
...
tesla_connection = teslajson.Connection(TESLA_USER, TESLA_PASSWORD)  #WK removed quotes from variable names
...
def GetCarTimezone(latitude, longitude):
    ...
    timezone_corrector = 0 #(location.rawOffset + location.dstOffset) / 3600 added by WK
...
def GetLocation():
    ...
    if (location.country == "US" or location.country == "CA"):
